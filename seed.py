"""seed.py — docker compose exec api python seed.py"""
...
def run():
    ...
    try:
        ...
        requester = User(id=1, name="Ana Souza", email="ana@bbts.com",
                        password_hash=hash_password("123456"), role=RoleEnum.REQUESTER)
        rh = User(id=2, name="Carlos RH", email="carlos@bbts.com",
                  password_hash=hash_password("123456"), role=RoleEnum.RH)
        ...
        raw = [
            ...
            {"full_name":"Fernanda Lima","headline":"Eng Backend Sênior | Java, Kafka","email":"fernanda@email.com","location":"São Paulo, SP",
             ...
             "langs":[("Inglês","Intermediário")],"certs":[],"edus":[("UNICAMP","Eng de Computação","Bacharelado",2016)],
             "exps":[("Magazine Luiza","Senior Backend",2018,None,True)]},
            {"full_name":"Bruno Martins","headline":"Dev Java Pleno | Spring Boot, AWS","email":"bruno@email.com","location":"Campinas, SP",
             ...
             "langs":[("Inglês","Básico")],"certs":[("AWS Cloud Practitioner","Amazon",2023)],
             "edus":[("PUC-Campinas","Sistemas de Informação","Bacharelado",2019)],
             "exps":[("Totvs","Dev Java Pleno",2020,None,True)]},
            {"full_name":"Juliana Costa","headline":"Backend Engineer | Python, Java","email":"juliana@email.com","location":"Recife, PE",
             ...
             "langs":[("Inglês","Intermediário")],"certs":[],"edus":[("UFPE","Ciência da Computação","Bacharelado",2017)],
             "exps":[("Tempest","Backend Eng",2018,None,True)]},
            {"full_name":"Lucas Ferreira","headline":"Engenheiro de Dados | Python, Spark, dbt","email":"lucas@email.com","location":"Remoto",
             ...
             "langs":[("Inglês","Avançado")],"certs":[("AWS Cloud Practitioner","Amazon",2022)],
             "edus":[("UFMG","Ciência da Computação","Bacharelado",2018)],
             "exps":[("Semantix","Data Engineer Sr",2020,None,True)]},
            {"full_name":"Camila Rocha","headline":"Dev Frontend Sênior | React, TypeScript","email":"camila@email.com","location":"São Paulo, SP",
             ...
             "langs":[("Inglês","Intermediário")],"certs":[],"edus":[("FIAP","Análise e Desenvolvimento","Tecnólogo",2017)],
             "exps":[("iFood","Frontend Sr",2020,None,True)]},
        ]
        ...
        v1 = Vacancy(title="Desenvolvedor Frontend Sênior", description="Liderar design system.",
                     location="São Paulo, SP (Híbrido)", priority=PriorityEnum.HIGH,
                     status=VacancyStatus.DRAFT, requester_id=1,
                     created_at=datetime(2024,6,1,9,0), updated_at=datetime(2024,6,1,9,0))
        db.add(v1); db.flush()
        ...
        v2 = Vacancy(title="Engenheiro de Dados Pleno", description="Pipelines e dashboards.",
                     location="Remoto", priority=PriorityEnum.MEDIUM,
                     status=VacancyStatus.PENDING_APPROVAL, requester_id=1,
                     created_at=datetime(2024,6,3,10,0), updated_at=datetime(2024,6,4,8,0))
        db.add(v2); db.flush()
        ...
        v3 = Vacancy(title="Tech Lead Backend (Java / Spring)", description="Liderança técnica do squad de pagamentos.",
                     location="São Paulo, SP (Presencial)", priority=PriorityEnum.CRITICAL,
                     status=VacancyStatus.APPROVED, requester_id=1,
                     created_at=datetime(2024,5,20,9,0), updated_at=datetime(2024,5,23,14,0))
        db.add(v3); db.flush()
        ...
        db.add(ApprovalDecision(vacancy_id=v3.id, rh_user_id=2, decision=DecisionEnum.APPROVED,
                                justification="Vaga estratégica Q3.", decided_at=datetime(2024,5,23,14,0)))
        ...
        db.add(IntegrationLog(source="JSON", filename="seed.json", status=IntegrationStatus.SUCCESS,
                              total_records=len(raw), success_count=len(raw), error_count=0))
        db.commit()
        print("✅ Seed concluído!")
        print("   ana@bbts.com / 123456 → REQUESTER")
        print("   carlos@bbts.com / 123456 → RH")
